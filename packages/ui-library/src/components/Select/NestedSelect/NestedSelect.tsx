      ...
      acc.push(
        <div style={{ paddingLeft: LEVEL_LEFT_MARGIN * level }}>
          <OptionItem
            ...
            labelLeftIconProps={children ? { Component: IconCaretDownFilled, size: 'xxsmall' } : undefined}
          />
        </div>
      );
    ...
    <div className="select select--multi" ref={setContainerRef}>
      ...
      {isDropdownOpen && (
        <div className="select__options" style={{ left, width, top: bottom }}>
          <div className="select__options__scroll scrollbar scrollbar--vertical">{generateFolders(options, 0)}</div>
        </div>
      )}
    </div>
  );
