...
export const Progress = (props: TProgressPropTypes): JSX.Element => {
  ...
  return (
    <div
      className={classNames('progress-bar', `progress-bar--${type}`, `progress-bar--${size}`, className, {
    ...
    >
      ...
      {!noText ? (
        <span className="progress-bar__percent">{stepCount ? `${currentStep}/${stepCount}` : `${percent}%`}</span>
      ) : null}
    </div>
  );
};
